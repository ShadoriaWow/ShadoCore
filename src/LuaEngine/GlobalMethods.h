...
namespace LuaGlobalFunctions
{
    ...
    int PerformIngameSpawn(Eluna* /*E*/, lua_State* L)
    {
        ...
        if (spawntype == 1) // spawn creature
        {
            if (save)
            {
                ...
                if (!creature->Create(eObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), map, phase, entry, 0, 0, x, y, z, o))
                {
                    delete creature;
                    Eluna::Push(L);
                    return 1;
        ...
        if (spawntype == 2) // Spawn object
        {
            ...
            if (!object->Create(lowguid, objectInfo->entry, map, phase, x, y, z, o, { }, 0, GO_STATE_READY))
            {
                delete object;
                Eluna::Push(L);
                return 1;
    ...
    int Ban(Eluna* /*E*/, lua_State* L)
    {
        ...
        {
            case BAN_ACCOUNT:
                if (!Battlenet::Utf8ToUpperOnlyLatin(nameOrIP))
                    return 0;
                break;
            case BAN_CHARACTER:
